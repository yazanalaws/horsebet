...
export default function LevelsCreator({ matchId  , setLevelsCreated}: Props) {
        ...
        level.id === levelId
          ? {
              ...
              horses: level.horses.map((horse, i) =>
                i === index ? { ...horse, [field]: field === 'price' ? Number(value) : value } : horse
              )
            }
          : level
      ) || null
  ...
  const createLevelHorses = async () => {
    ...
    // Optionally validate
    const invalid = payload.find(h => !h.name || isNaN(h.price) || h.price < 0)
    if (invalid) {
            ...
            {level.horses.map((horse, i) => (
              ...
              >
                <input
                  ...
                  value={horse.name}
                  onChange={e => updateHorse(level.id, i, 'name', e.target.value)}
                />
                <input
                  type='number'
                  inputMode='decimal'
